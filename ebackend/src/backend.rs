...
//! copy from  https://github.com/Gip-Gip/egui-plotter
//! [![MIT License](https://img.shields.io/badge/license-MIT-blue.svg)](./LICENSE.txt)
//! 在原有基础上增加了 draw_circle,  draw_rect
//! 
...
impl<'a> DrawingBackend for EguiBackend<'a> {
    ...
    /**
    实现Trait DrawingBackend的draw_line方法, 在egui UI中绘制线
    */
    ...
    /**
    实现Trait DrawingBackend的draw_text方法, 在egui UI中打印文本
    */
    ...
    /**
    实现Trait DrawingBackend的fill_polygon方法, 在egui UI中绘制多边形
     */
    ...
    /**
    实现Trait DrawingBackend的draw_circle方法, 在egui UI中绘制圆形
     */
    fn draw_circle<S: BackendStyle>(
        ...
        let p0 = self.point_transform(EguiBackendCoord::from(center), bounds);
      
        let color: Color32 = EguiBackendColor::from(style.color()).into();
        let stroke = Stroke::new(style.stroke_width() as f32, color);

       if fill {
           painter.circle_filled(p0.into(), radius as f32, color);
       }else{
           painter.circle_stroke(p0.into(), radius as f32, stroke); 
       } 
       Ok(())
    }

    /**
    实现Trait DrawingBackend的draw_rect方法, 在egui UI中绘制矩形
    */
    fn draw_rect<S: BackendStyle>(
        ...
        let color: Color32 = EguiBackendColor::from(style.color()).into();
        let stroke = Stroke::new(style.stroke_width() as f32, color);
        let rect = Rect::from_points(&[p0.into(), p1.into()]);
       if fill {
           painter.rect_filled(rect, Rounding::default(), color);
       }else{
           painter.rect_stroke(rect, Rounding::default(), stroke);  //circle_stroke(p0.into(), radius as f32, stroke); 
       } 
       Ok(())
    }
